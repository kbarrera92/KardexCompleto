...
Public Class frmVerDetalleCompra

    Sub updateDet()
        Dim sqlupdate As String = "UPDATE DETALLECOMPRA SET cantidad = @c, precio = @p, subtotal = @subt WHERE (nDetalleC = @nd AND nCompra = @nv)"
    ...
    Private Sub frmVerDetalleCompra_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        If nombreRol = "VENDEDOR" Then
            DataGridView1.Columns(2).ReadOnly = True
            DataGridView1.Columns(3).ReadOnly = True
            DataGridView1.Columns(4).ReadOnly = True
            fillDGVSP("getDetalleCompra", DataGridView1, Me, CInt(TextBox1.Text))
        Else
            fillDGVSP("getDetalleCompra", DataGridView1, Me, CInt(TextBox1.Text))
        End If
    End Sub
    ...
    Private Sub DataGridView1_KeyDown(sender As Object, e As KeyEventArgs) Handles DataGridView1.KeyDown
        If e.KeyCode = Keys.Delete Then
            If MessageBox.Show("¿Desea eliminar este registro?", "Eliminar", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
                Dim sqldeldet As String = "DELETE FROM DETALLECOMPRA WHERE (nDetalleC = @nd AND nCompra = @nc)"
                Dim cmd As SqlCommand
                Try
                    cmd = New SqlCommand(sqldeldet, conn)
                    cmd.Parameters.AddWithValue("nd", CInt(DataGridView1.CurrentRow.Cells(0).Value))
                    cmd.Parameters.AddWithValue("nc", CInt(TextBox1.Text))
                    openConnection()
                    cmd.ExecuteNonQuery()
                    closeConnection()
