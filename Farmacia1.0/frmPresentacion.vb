...
Public Class frmPresentacion
    ...
    Dim correlativo As String = "SELECT IDENT_CURRENT ('PRESENTACION') AS Current_Identity"
    Dim sqlPres As String = "SELECT idPresentacion, presentacion FROM PRESENTACION"

    Sub getDatos()
        Dim sql As String = "SELECT * FROM PRESENTACION WHERE idPresentacion = @id"
    ...
    Private Sub frmPresentacion_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ListBox1.DataSource = updateList(sqlPres)
        ListBox1.ValueMember = updateList(sqlPres).Columns(0).ToString
        ListBox1.DisplayMember = updateList(sqlPres).Columns(1).ToString
    End Sub
    ...
    Private Sub Button2_Click(sender As Object, e As EventArgs) Handles Button2.Click
        If RegOAct = 1 Then
            ...
            Else
                If MessageBox.Show("¿Desea guardar este registro?", "Guardar", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
                    ...
                    Dim sql As String = "INSERT INTO PRESENTACION VALUES(@pres)"
                    Dim cmd As SqlCommand
                    cmd = New SqlCommand(sql, conn)
                    ...
                    Finally
                        closeConnection()
                        With ListBox1
                            .DataSource = updateList(sqlPres)
                            .ValueMember = updateList(sqlPres).Columns(0).ToString
                        End With
        ...
        Else
            If MessageBox.Show("¿Desea guardar los cambios de este registro?", "Guardar cambios", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
                Dim sqlupdate As String = "UPDATE PRESENTACION SET presentacion = @pres WHERE idPresentacion = @id"
                Dim cmd As SqlCommand
    ...
    Private Sub Button3_Click(sender As Object, e As EventArgs) Handles Button3.Click
        If MessageBox.Show("¿Desea eliminar este registro?", "Eliminar", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
            Dim sqldelete As String = "DELETE FROM PRESENTACION WHERE idPresentacion = @id"
